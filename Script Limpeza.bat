...
schtasks /delete /tn "LimpezaPosReparo" /f
...
REM ******************** TEMP WINDOWS ********************
del C:\Windows\Temp\*.* /f /s /q >nul 2>&1
for /d %%x in (C:\Windows\Temp\*) do rd /s /q "%%x" >nul 2>&1

REM ******************** TEMP USUARIOS ********************
for /d %%F in (C:\Users\*) do (
    del %%F\AppData\Local\Temp\*.* /f /s /q >nul 2>&1
    for /d %%x in (%%F\AppData\Local\Temp\*) do rd /s /q "%%x" >nul 2>&1
)

REM ******************** WINDOWS UPDATE CACHE ********************
del C:\Windows\SoftwareDistribution\Download\*.* /f /s /q >nul 2>&1
del C:\Windows\SoftwareDistribution\DataStore\*.* /f /s /q >nul 2>&1

REM ******************** DELIVERY OPTIMIZATION ********************
del C:\Windows\SoftwareDistribution\DeliveryOptimization\*.* /f /s /q >nul 2>&1
for /d %%x in (C:\Windows\SoftwareDistribution\DeliveryOptimization\*) do rd /s /q "%%x" >nul 2>&1

REM ******************** LOGS WINDOWS ********************
del C:\Windows\Logs\*.* /f /s /q >nul 2>&1
...
:: ******************** LOG ********************
set LOG=C:\otimizacao_windows.log
echo Iniciando otimizacao em %date% %time% > %LOG%

:: ******************** TRIM SSD ********************
echo.
echo [1/4] Executando TRIM (SSD)...
echo [1/4] TRIM SSD... >> %LOG%
defrag C: /L >> %LOG% 2>&1

:: ******************** OTIMIZACAO GERAL DO VOLUME ********************
echo.
echo [2/4] Otimizando volume...
echo [2/4] Otimizacao de volume... >> %LOG%
defrag C: /O >> %LOG% 2>&1

:: ******************** VERIFICAR TRIM ATIVO ********************
echo.
echo [3/4] Verificando TRIM...
echo [3/4] Verificando TRIM... >> %LOG%
fsutil behavior query DisableDeleteNotify >> %LOG% 2>&1

:: ******************** LIMPEZA DE MEMORIA STANDBY (opcional leve) ********************
echo.
echo [4/4] Limpando cache de memoria (standby)...
echo [4/4] Limpando memoria... >> %LOG%
